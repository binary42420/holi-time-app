...
interface ProcessResult {
  status: 'compressed' | 'would_compress' | 'skipped' | 'error';
  reason?: string;
 ...
 */
async function compressImage(inputBuffer: Buffer, originalMimeType: string): Promise<CompressionResult> {
  try {
 ...
 */
async function processUserAvatar(user: { id: string; name: string | null; email: string; avatarData: string | null }): Promise<ProcessResult> {
  ...
  try {
    ...
    console.error(`   ❌ Failed to process avatar:`, errorMessage);
 ...
 */
export async function compressAllAvatars(): Promise<void> {
  ...
  try {
    ...
    const results = {
      ...
      details: [] as Array<{ user: string | null } & ProcessResult>
    };
